...
def process_rank(infile, outdir, rank, include, exclude=None, remove_pattern=None):
    ...
    The logic mimics:
      paste -d, <( ... | cut -f1,3 | tr '\t' ',' | sed "s/.*<include>//g") \
                  <( ... | cut -f1)
    and then prepends a header.
    ...
    with open(infile, 'r', encoding='utf-8') as fin:
        for line in fin:
            ...
            fields = line.rstrip('\n').split('\t')
            # We expect at least three columns (fields[0] and fields[2]).
            if len(fields) < 3:
                continue
            # Mimic: cut -f1,3 | tr '\t' ','
...
def combine_csv_to_xlsx(outdir, prefix, levels):
    ...
    Each CSV file's name (e.g. "kingdom.csv") is used (without extension) as the sheet name.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Process taxonomy TSV file into CSVs and optionally combine them into an XLSX workbook."
    )
    parser.add_argument("infile", help="Input TSV file")
    parser.add_argument(
        "--outdir",
        default=None,
        help="Output directory (default: '<prefix>-taxa')"
    )
    parser.add_argument(
        "--combine",
        action="store_true",
        help="Combine CSV files into a single XLSX workbook"
    )
    ...
    # Each entry is a tuple: (Rank, include_pattern, exclude_pattern, [optional remove_pattern])
    # For species_with_strain, we assume no exclusion and use a different remove_pattern if needed.
    levels = [
        ("Kingdom", "k__", "p__"),
        ("Phyla", "p__", "c__"),
        ("Class", "c__", "o__"),
